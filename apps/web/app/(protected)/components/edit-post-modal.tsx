...
import { MapPin, Globe, Image as ImageIcon, Lock } from 'lucide-react';
...
export function EditPostModal({ post, open, onOpenChange, onSuccess }: EditPostModalProps) {
  ...
  useEffect(() => {
    if (open && post) {
      ...
      const mUrl = post.media?.[0]?.mediaUrl || '';
      setMediaUrl(mUrl);
      setShowMediaInput(!!mUrl);
    }
  }, [open, post]);
        ...
        <div className="grid gap-4 py-4">
          ...
          {showMediaInput && (
            <div className="animate-in fade-in slide-in-from-top-2 duration-200">
              <Input
                placeholder="Dán đường dẫn hình ảnh (URL) vào đây..."
                value={mediaUrl}
          ...
          <div className="border-t pt-4 flex gap-2 flex-wrap items-center">
            ...
            >
              ...
              <span className="capitalize">{status === 'public' ? 'Công khai' : 'Chỉ mình tôi'}</span>
            </Button>
          </div>
        </div>
        <DialogFooter>
